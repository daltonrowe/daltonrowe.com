...
test("decodes all three input formats to rgba", async () => {
  for (const file of [JPEG, PNG, WEBP]) {
    const image = await render(source({ file }));

    assert.ok(image.width > 0 && image.height > 0, `${file} has no size`);
    assert.equal(image.data.length, image.width * image.height * 4, `${file} is not tightly packed rgba`);
  }
});
...
test("jpeg and webp encode to their own containers", async () => {
  const image = await render(resize(source({ file: PNG }), { width: 64 }));

  const jpeg = await encode(image, "jpeg", { quality: 70 });
  const webp = await encode(image, "webp", { quality: 70 });

  assert.deepEqual([...jpeg.subarray(0, 3)], [0xff, 0xd8, 0xff]);
  assert.equal(webp.toString("ascii", 0, 4), "RIFF");
  assert.equal(webp.toString("ascii", 8, 12), "WEBP");

  // and both come back at the size they went in
  assert.equal((await decode(jpeg)).width, 64);
  assert.equal((await decode(webp)).width, 64);
});
...
test("jpeg output is flattened, not left to guess", async () => {
  ...
  const buffer = await toBuffer(transparent, { format: "jpeg", background: "#ffffff", quality: 90 });
  const decoded = await decode(buffer);

  assert.ok(decoded.data[0] > 240, `expected white, got ${decoded.data[0]}`);
});
